...
now = datetime.now()
print "It's on bitches"
print '%s/%s/%s %s:%s' % (now.month, now.day, now.year, now.hour, now.minute)
time.sleep(1)
hint = "Don't forget to include the correct units! \nExiting..."
#the \n prints it on a seperate line 
option = raw_input("What shape do you want?").upper()
if option == "C":
	radius = float(raw_input ("what size radius"))
  	area = pi*(radius**2)
	...
	base = float(raw_input ("what size base"))
	height = float(raw_input ("what size height"))
...
then the giant if/else calculates the area of the circle by asking for a radius length and printing that to two decimal points (%.2f (floating point) as well as the hint)"""
